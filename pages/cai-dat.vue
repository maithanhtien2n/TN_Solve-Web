...
const displayApiKey = computed(
  () => apiKey.value || userData.value?.apiKey || "",
);
...
watch(
  () => userData.value?.settings?.usePersonalResource,
  (val) => {
...
function timeAgo(dateStr: string): string {
  ...
  if (diff < 365 * 86400)
    return `${Math.floor(diff / (30 * 86400))} tháng trước`;
...
onMounted(async () => {
  ...
  const res = await fetch(
    `${base}/api/common/setting-file-info?title=${encodeURIComponent("File build Extensions TN Solve")}`,
  );
    ...
    <div class="sw-card">
          ...
          <div class="sw-row-left">
            ...
            <div>
              ...
              <div
                ...
                "
              >
                <v-icon size="11" style="margin-top: 1px"
      ...
      <div class="sw-row sw-row--apikey">
        ...
        <div v-if="displayApiKey" class="sw-apikey-wrap">
          ...
          <button
            class="sw-apikey-btn"
            :class="{ 'sw-apikey-btn--ok': apiKeyCopied }"
            title="Sao chép"
            @click="onCopyApiKey"
          >
            <v-icon size="15">{{
  ...
  <v-dialog v-model="confirmRegenerate" max-width="400">
    <v-card rounded="xl" elevation="0" border>
      <div class="sw-dialog-head">
        <div class="sw-dialog-icon">
          <v-icon color="white" size="15">mdi-alert-outline</v-icon>
        </div>
        <div>
      ...
      <div class="sw-dialog-body">
        Key cũ sẽ bị
        <strong style="color: #dc2626">vô hiệu hóa ngay lập tức</strong>. Mọi
        ứng dụng đang dùng key cũ sẽ ngừng hoạt động.
      </div>
      <div class="sw-dialog-foot">
        <button class="sw-btn-ghost" @click="confirmRegenerate = false">
          Hủy
        </button>
        <button
          class="sw-btn-primary"
        ...
        >
          ...
          />
          <template v-else
            ><v-icon size="14">mdi-refresh</v-icon> Xác nhận</template
          >
        </button>
      </div>
    </v-card>
