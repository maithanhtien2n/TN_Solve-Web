...
async function loadItems(event: any) {
  ...
  try {
    const res = await masterDataService.getOtherProductsAdmin({
      search: event?.search,
      page: event?.page,
      limit: event?.limit,
    });
    ...
    console.log("Lỗi khi tải danh sách sản phẩm khác!", error);
  } finally {
    loading.value = "";
  }
}
...
const onSubmitForm = handleSubmit(async (values: any) => {
  ...
  try {
    if (values._id) {
      await masterDataService.updateOtherProduct({
        _id: values._id,
        title: values.title,
        value: values.value,
      });
    } else {
      await masterDataService.createOtherProduct({
        title: values.title,
        value: values.value,
      });
    }
    ...
    console.log("Lỗi khi lưu sản phẩm khác!", error);
  } finally {
    loading.value = "";
  }
});
...
const onAction = async (event: any) => {
  if (event.action === "add") {
    ...
    try {
      await masterDataService.deleteOtherProduct({ _id: event.ids[0] });
      dataTableRef.value?.loadItems();
    } catch (error) {
      console.log("Lỗi khi xóa sản phẩm khác!", error);
    } finally {
      loading.value = "";
    }
  }
...
async function onToggleStatus(item: any) {
  ...
  try {
    ...
    console.log("Lỗi khi đổi trạng thái sản phẩm khác!", error);
  } finally {
    loading.value = "";
  }
}

definePageMeta({ layout: "admin", title: "Sản phẩm khác" });
</script>

<template>
  <CommonDialog
    ref="commonDialogRef"
    :title="_id.value.value ? 'Cập nhật sản phẩm khác' : 'Thêm sản phẩm khác'"
    width="500"
  >
    <div class="mt-2">
      <v-row>
        <v-col cols="12">
          <v-text-field
            ...
            variant="outlined"
            label="Tiêu đề (✳)"
            placeholder="Ví dụ: TN Solve Teacher, Tải công cụ TN Solve..."
        ...
        <v-col cols="12">
          <v-text-field
            ...
            variant="outlined"
            label="Đường dẫn (✳)"
            placeholder="Ví dụ: https://teacher.tnsolve.com"
