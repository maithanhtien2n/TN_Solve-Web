      ...
      >
        ...
        <div v-else-if="h.key == 'action'" class="d-flex justify-center">
          <v-btn
            ...
            size="40"
            variant="text"
            :disabled="Boolean((slotProps.item as any)?.isDelete || ['❌ Hình ảnh tải lên vi phạm chính sách!','❌ Cookies flow (veo3) của bạn không hợp lệ!','❌ Cookies flow (veo3) của bạn đã hết hạn!','❌ Hình ảnh tải lên vi phạm chính sách, vui lòng chọn ảnh khác!','❌ Thước phim liên quan đến chủ đề nhạy cảm, vi phạm chính sách nội dung!'].includes((slotProps.item as any)?.lastMessage?.note))"
